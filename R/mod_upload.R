  ...
  tagList(
    ...
    tabPanel(
      ...
      selectInput(
        ...
        label   = 'Object Source*: ',
        choices = list('Upload from PC' = 'pcfile'),
        width   = '200px'
      ),
      ...
      actionButton(ns("runLoadPrevious"), "Load data", icon = icon("play-circle")),
...
#'
#' @noRd
mod_upload_server <- function(id, data){
      ...
      c(input$previous_object_input),
      {
        ...
        outLoad <- eventReactive(input$runLoadPrevious, {
          if(input$previous_object_input == 'cloudfile'){ # upload from cloud
            ...
            result <- readRDS( file.path( getwd(),res_auth$repository,input$previous_input ) ) # old dataset
          }else if(input$previous_object_input == 'pcfile'){ # upload rds
            req(input$previous_object_file)
            result <- readRDS(input$previous_object_file$datapath)
          }
